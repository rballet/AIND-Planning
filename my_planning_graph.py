...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if a_node.prenodes.issubset(self.s_levels[level]):
                ...
                a_node.parents = set(node for node in self.s_levels[level] if node in a_node.prenodes)
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level-1]:
            for eff_nodes in a_node.effnodes:
                self.s_levels[level].add(eff_nodes)
                a_node.children.add(eff_nodes)
                eff_nodes.parents.add(a_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if (any([node in node_a2.action.effect_rem for node in node_a1.action.effect_add]) or
            any([node in node_a1.action.effect_rem for node in node_a2.action.effect_add])):
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if (any([node in node_a2.action.precond_neg for node in node_a1.action.effect_add]) or
            any([node in node_a2.action.precond_pos for node in node_a1.action.effect_rem]) or
            any([node in node_a1.action.precond_neg for node in node_a2.action.effect_add]) or
            any([node in node_a1.action.precond_pos for node in node_a2.action.effect_rem])):
            return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for level in range(len(self.s_levels)):
                s_level = [node.literal for node in self.s_levels[level]]
                if goal in s_level:
                    level_sum += level
                    break
        return level_sum
